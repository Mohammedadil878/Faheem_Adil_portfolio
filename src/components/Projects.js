        ...
        <section className='pt-20' id='projects'>
            ...
            <div className="grid grid-col-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                { PROJECTS.map((project) => {
                    return <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }} whileHover={{ scale: 1.05 }} key={project.id} className='group relative overflow-hidden rounded-3xl'>
                        ...
                        <motion.div initial={{ opacity: 0 }} whileHover={{ opacity: 1 }} transition={{ duration: 0.5 }} className="absolute inset-0 flex flex-col items-center justify-center text-white opacity-0 backdrop-blur-lg transition-opacity duration-500 group-hover:opacity-100">
                            ...
                            <a href={project.githubLink} target='_blank' rel='noopenner noreferrer' className='rounded-full bg-white px-4 py-2 text-black hover:bg-gray-300'>
